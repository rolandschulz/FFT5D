...
Options:\n\
  ...
  -O        use YZ order (default: ZY order)\n\
	...
	if (!(flags&FFT5D_BACKWARD)) { //write in as standard X,Y,Z
		...
		if (ccheck) {
			for(x=0;x<N;x++) {  
				for (y=0;y<fmin(M0,M-coor[0]*M0);y++) { // 2nd of fmin for non dividable
					for (z=0;z<fmin(K1,K-coor[1]*K1);z++) { 
						lin[x+y*N+z*N*M0]=in[x+(coor[0]*M0+y)*N+(coor[1]*K1+z)*N*M];  
					}
				}
			}} else {
		...
		if (!(flags&FFT5D_ORDER_YZ)) {
			...
			if (ccheck) {
				for(z=0;z<K;z++) {  
					for (x=0;x<fmin(N0,N-coor[0]*N0);x++) { //  2nd of fmin for non dividable
						for (y=0;y<fmin(M1,M-coor[1]*M1);y++) { 
							lin[z+x*K+y*K*N0]=in[(x+coor[0]*N0)+(y+coor[1]*M1)*N+z*N*M]; 
						}
					}
				}
			} else {
			...
			if (ccheck) {
				for(y=0;y<M;y++) {  
					for (z=0;z<fmin(K0,K-coor[0]*K0);z++) { //  2nd of fmin for non dividable
						for (x=0;x<fmin(N1,N-coor[1]*N1);x++) { 
							lin[y+z*M+x*M*K0]=in[(x+coor[1]*N1)+y*N+(z+coor[0]*K0)*N*M];  
						}
					}
				}
			} else {
